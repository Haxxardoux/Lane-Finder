...
def process_frame(img):
    ...
    left_right_bounds = np.zeros((1, 3, 2), dtype=np.float32)
    left_right_bounds[0, 0, 0] = 0.25
    left_right_bounds[0, 0, 1] = 0.5
    left_right_bounds[0, 1, 1] = 0.1
    left_right_bounds[0, 2, 1] = 0.1
    valid_bounds = np.zeros((0, 3, 2), dtype=np.float32)

    # Fit regression.
    l2_scales = np.array([0.005, 0.005], dtype=np.float32)
    # l2_scales=None
    res_threshold = 0.01
    n_prefits = 5000
    max_trials = 500000
    lanes_ransac = SLanesRANSACRegressor(residual_threshold=res_threshold, n_prefits=n_prefits, max_trials=max_trials, l2_scales=l2_scales)
    lanes_ransac.fit(1, 1, 960, 540, left_right_bounds)
